...
const signup = (req, res) => {
  ...
  AdModel.findOne({ email }, (err, result) => {
...
const login = (req, res) => {
  ...
  if (!email || !password)
    return res.status(400).send("필수값이 입력되지 않았습니다.");
  AdModel.findOne({ email }, (err, user) => {
    ...
    bcrypt.compare(password, user.password, (err, isMatch) => {
      ...
      AdModel.findByIdAndUpdate(user._id, { token }, (err, result) => {
...
const checkAuth = (req, res, next) => {
  ...
  jwt.verify(token, "secretToken", (err, _id) => {
    ...
    AdModel.findOne({ _id, token }, (err, result) => {
      if (err)
...
const logout = (req, res) => {
  ...
  jwt.verify(token, "secretToken", (err, _id) => {
    ...
    AdModel.findByIdAndUpdate(_id, { token: "" }, (err, result) => {
      if (err) return res.status(500).send("로그아웃 시 오류가 발생했습니다");
      res.clearCookie("token");
      res.redirect("/");
    });
